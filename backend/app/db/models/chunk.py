...
class Chunk(Base):
    ...
    __table_args__ = (
        ...
        # GIN over the tsvector: the index type full-text search requires.
    ...
    # Generated by Postgres from content, so it can never drift out of step with
    # the text it indexes — an application-maintained column would need updating
    # everywhere content is written, and one missed path is a silently unsearchable
    # chunk.
    #
    # TWO configurations, because one cannot serve both jobs.
    #
    # 'english' stems and removes stop words, so "processing" matches "processed".
    # That is what makes prose searchable — and it is exactly wrong for
    # identifiers: it splits "E-402" into 'e' and '-402' at separate positions,
    # while the query parser demands them adjacent, so the two never match.
    #
    # 'simple' does no stemming, and keeps "E-402" matchable. Concatenating both
    # vectors means prose queries hit the stemmed tokens and identifier queries
    # hit the literal ones, with no need to guess which kind a query is.
